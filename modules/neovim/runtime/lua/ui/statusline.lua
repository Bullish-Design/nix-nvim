...
-- The active loci workspace (tab-scoped, set by `require("loci").activate`). Presentational + pcall-guarded;
-- empty (and so invisible) whenever no loci workspace is active on the tab.
local function section_loci(trunc_width)
  if vim.o.columns < (trunc_width or 120) then
    return ""
  end
  local ok, wid = pcall(function()
    return vim.t.loci_workspace_id
  end)
  if ok and type(wid) == "string" and wid ~= "" then
    return "loci:" .. wid
  end
  return ""
end
...
local active = function()
  ...
  return statusline.combine_groups({
    ...
    " ",
    "%<",
    { hl = "MiniStatuslineDevinfo", strings = { git, loci } },
    "%#StlSepDevinfoFile#",
    " ",
